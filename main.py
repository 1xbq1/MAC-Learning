...
def min_matrix(data_matrix):
    (rows, cols) = data_matrix.size()
    values = torch.zeros(rows)
    indices = torch.zeros(rows)
    for i in range(rows):
...
def get_W(Z):
    ...
    w1 = torch.mm(Z.to(torch.float32), torch.diag(power_vector).to(torch.float32))

    # now put this power vector as the diagonal of matrix of zeros; multiple by the transpose
    return torch.mm(w1, Z.t().to(torch.float32))
...
class Processor():
    ...
    def __init__(self, arg):
        ...
        self.global_step = 0
        self.load_data()
        #self.z_, self.w_ = self.anchor_graph()
        self.load_model()
        self.load_optimizer()
    ...
    def load_model(self):
        ...
        #self.model = nn.DataParallel(Model(**self.arg.model_args), device_ids=self.arg.device)
        print(self.model)
    ...
    def anchor_graph(self, features): # 2batchsize * num_class (16 * 60)
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            data_all = torch.cat((data_all, data), dim=0) if data_all is not None else data
        tn, _, _, _, _ = data_all.size()
        data_all = data_all.view(tn, -1)'''
        
        k = 60 # number of anchors
        s = 6 # it's how many closest anchors we look for each sample
        #anchors = kmeans(features, k) # anchor (k*num_class)
        ...
        #print('features.size():{}, anchors.size():{}'.format(features.size(), anchors.size()))
        #print('features.t().size():{}, anchors.t().size():{}'.format(features.t().size(), anchors.t().size()))
        z_ = get_Z(features.t(), anchors.t(), s).cuda(self.output_device)
        w_ = get_W(z_).cuda(self.output_device)
        #print('z_.size():{}, w_.size():{}'.format(z_.size(), w_.size()))
        _, z_index = z_.max(1)
        z_index = z_index.unsqueeze(1)
        #print('z_index.size():{}'.format(z_index.size()))
        return z_index, w_ # z_(2batchsize, k)  w_(2batchsize, 2batchsize) z_index(2batchsize, 1)
    
    def info_nce_loss(self, features, index): # anchor contrastive loss
        #global z_, w_
        '''print("index", index.size())
        print("self.z_", self.z_.size())
        print("self.w_", self.w_.size())'''
        z_1 = self.z_[index]
        #print("z_1",z_1.size())
        z_ = z_1.repeat(2,1)
        w_1 = self.w_[index]
        ...
        features = F.normalize(features, dim=1)
        #similarity_matrix = (w_.cuda(self.output_device).mul(torch.matmul(features, features.T)))/0.07 #逐点相乘
        ...
        #self.print_log('z_.size():{}'.format(z_.size()))
        #self.print_log('similarity_matrix.size():{}'.format(similarity_matrix.size()))
        #self.print_log('z_last.size():{}, z_last:{}'.format(z_last.size(), z_last))
        #self.print_log('similarity_matrix.size():{}, similarity_matrix:{}'.format(similarity_matrix.size(), similarity_matrix))
        #self.print_log('similarity_matrix[z_last.bool()].size():{}, similarity_matrix[z_last.bool()]:{}'.format(similarity_matrix[z_last.bool()].size(), similarity_matrix[z_last.bool()]))
        
        positives = similarity_matrix.mul(z_last)
            
        #softmax = torch.sum(torch.exp(positives), dim=1).reshape(-1,1)/torch.sum(torch.exp(similarity_matrix), dim=1).reshape(-1,1)
    ...
    def info_nce_loss_i(self, featuresi, features, index):
        ...
        for v in range(V):
            ...
            #anchor graph z_, w_
            #z_, w_ = self.anchor_graph(features_)
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            #global z_, w_
            V, _, _ = i_a.size()
            features_sum = g_a + t_a + g_s + t_s
            for v in range(V):
                features_sum = features_sum + i_a[v]
                features_sum = features_sum + i_s[v]
            features_all = torch.cat((features_all, features_sum), dim=0) if features_all is not None else features_sum
            #self.z_, self.w_ = self.anchor_graph(torch.cat((features_sum,features_sum),dim=0))
            ...
            if epoch == 0:
                ...
                #anchor graph z_tga, w_tga
                #z_tga, w_tga = self.anchor_graph(features_tga)
                con_loss_aa_tg = self.info_nce_loss0(features_tga)
                            
                features_tgs = torch.cat((t_s, g_s), dim=0)
                #anchor graph z_tgs, w_tgs
                #z_tgs, w_tgs = self.anchor_graph(features_tgs)
            ...
            #con_loss = self.loss(logits, labels)
            ...
            i_a = sum_i_a / V
            i_s = sum_i_s / V
            output = i_a + g_a + t_a + g_s + t_s + i_s
            N = label.size(0)
        ...
        #print("features_all", features_all.size())
        with torch.no_grad():
            if (epoch % 20) == 0:
                self.z_, self.w_ = self.anchor_graph(features_all)
        ...
        self.print_log(
            '\tlearning rate: {:.4f}.'.format(self.lr))
        self.print_log(
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            if accuracy > self.best_acc:
                ...
                weights = OrderedDict([[k.split('module.')[-1],
                                        v.cpu()] for k, v in state_dict.items()])
                torch.save(weights, self.arg.model_saved_name + '_best' + '.pt')
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            self.print_log('best accuracy:{}, model_name:{}'.format(self.best_acc, self.arg.model_saved_name))
