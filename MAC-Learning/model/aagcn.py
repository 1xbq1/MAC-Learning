...
class Encoder(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0, adaptive=True, attention=False, pooling=False):
    ...
    def forward(self, x):
        ...
        if self.pooling:
            ...
            x = x.view(N, M, c_new, t_new, V)
            x = x.mean(3).mean(1)
            x = self.drop_out(x)
            x = x.view(N*V, -1)
            x = self.fc(x)
            # x_i = x.view(N, V, -1).permute(1, 0, 2).contiguous()
            x_i = x.view(N, V, -1)
            x = x_i.mean(1)
            return x_i, x
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        #encoder1-global-f  encoder2-local-f  encoder3-context-f  encoder4-context-h  encoder5-local-h  encoder6-global-h
        # self.encoder1 = Encoder(num_class=num_class, num_point=num_point, num_person=num_person, graph=graph, graph_args=graph_args, in_channels=in_channels,
        #                        drop_out=drop_out, adaptive=True, attention=False, pooling=True)
        self.encoder2 = Encoder(num_class=num_class, num_point=num_point, num_person=num_person, graph=graph, graph_args=graph_args, in_channels=in_channels,
                               drop_out=drop_out, adaptive=True, attention=False, pooling=False)
        self.encoder3 = Encoder(num_class=num_class, num_point=num_point, num_person=num_person, graph=graph, graph_args=graph_args, in_channels=in_channels,
                               drop_out=drop_out, adaptive=True, attention=True, pooling=True)
        self.encoder4 = Encoder(num_class=num_class, num_point=num_point, num_person=num_person, graph=graph, graph_args=graph_args, in_channels=in_channels,
                               drop_out=drop_out, adaptive=False, attention=True, pooling=True)
        self.encoder5 = Encoder(num_class=num_class, num_point=num_point, num_person=num_person, graph=graph, graph_args=graph_args, in_channels=in_channels,
                               drop_out=drop_out, adaptive=False, attention=False, pooling=False)
        # self.encoder6 = Encoder(num_class=num_class, num_point=num_point, num_person=num_person, graph=graph, graph_args=graph_args, in_channels=in_channels,
        #                        drop_out=drop_out, adaptive=False, attention=False, pooling=True)

    def forward(self, x):
        
        # g_a = self.encoder1(x)
        i_a, g_a = self.encoder2(x)
        t_a = self.encoder3(x)
        
        t_s = self.encoder4(x)
        i_s, g_s = self.encoder5(x)
        # g_s = self.encoder6(x)
        

        return i_a, g_a, t_a, i_s, g_s, t_s
